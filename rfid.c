...
void spi_init() {
	...
	// Turn antenna on
	mfrc522_write_register(REG_TX_CONTROL, mfrc522_read_register(REG_TX_CONTROL) | 0x03);
}
...
void mfrc522_write_register(uint8_t reg, uint8_t value) {
    ...
    spi_transfer(((reg << 1) & 0x7E) | 0x80); // Send register address with read command
...
void mfrc522_send_command(uint8_t cmd) {
    mfrc522_write_register(0x01, (mfrc522_read_register(0x01) & 0xF0) | (cmd & 0x0F)); // Write to CommandReg register
}
...
void mfrc522_read_uid(uint8_t *uid_buffer) {
    ...
    mfrc522_send_command(CMD_MEM); // Store 25 bytes into internal buffer
    for (uint8_t i = 0; i < 4; i++) {
        mfrc522_write_register(REG_FIFO_DATA, tag_uid[i]); // Write UID bytes to FIFO
    }
...
uint8_t mfrc522_check_card_present() {
    ...
    mfrc522_send_command(CMD_RECEIVE); // Activate the transmitter to start scanning
    _delay_ms(1); // Wait for scanning to stabilize
    status = mfrc522_read_register(REG_STATUS1); // Read Status1 register
    return (status & (1 << 5)) ? 1 : 0; // Check CRCReady bit (bit 5) for card presence
}
