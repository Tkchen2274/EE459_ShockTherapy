...
void lcd_init(void) {
  ...
  unsigned char status =
      i2c_io(LCD_ADDR, (unsigned char[]){0x80, 0x38}, 2, NULL, 0);
  _delay_ms(1);
...
void clear_screen(void) { // function to clear LCD screen
  unsigned char status = i2c_io(LCD_ADDR, (unsigned char[]){0x80, 0x01}, 2,
                                NULL, 0); // 80 is control byte, 01 means clear
};
...
void lcd_moveto(unsigned char pos) {
  unsigned char data[2] = {
      0x80, (pos | 0x80)}; // creates command for moving to specified pos
