...
ISR(USART_RX_vect) {
  unsigned char received_data =
      UDR0; // Read the received data into a global variable

  if ((byte_count == 0) && (received_data == 0x01)) { // Receiving face result
    face_flag = 1;
    byte_count++;
  } else if ((byte_count == 0) &&
             (received_data == 0x02)) { // Receiving fingerprint result
    finger_flag = 1;
    byte_count++;
  } else if ((byte_count == 0) &&
             (received_data == 0x03)) { // Receiving rfid result
    rfid_flag = 1;
    byte_count++;
  }
  // 0x04 passwd 2 bytes
  ...
  // 0x04 arduino to rpi request pic for intruder
           ...
           (received_data == 0x04)) { // receiving updated password
    passwd_flag = 1;
    byte_count++;
  } else if ((byte_count == 0) &&
             (received_data == 0x05)) { // receiving message of selected auths
    auth_flag = 1;
    byte_count++;
  } else if ((byte_count == 0) &&
             (received_data == 0x06)) { // receiving message to lock/unlock
    lock_flag = 1;
    byte_count++;
  } else if ((byte_count == 0) &&
             (received_data ==
              0x07)) { // receiving message that touch has been added
    addedtouch_done = 1;
  } else if ((byte_count == 0) &&
             (received_data ==
              0x08)) { // receiving message that touch has been added
    addedrfid_done = 1;
