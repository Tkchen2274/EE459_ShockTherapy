...
int main(void) {
  ...
  unsigned char auth_byte =
      EEPROM_read(1000); // EEPROM read of authentication byte
  unsigned int password =
      EEPROM_read(1002); // Least signficant 8 bits of password

  password |= EEPROM_read(1001) << 8; // Most signifcant 8 bits of password

  unsigned char pass[4] = {password / 1000, (password / 100) % 10,
                           (password / 10) % 10,
                           password % 10}; // array for holding password
  ...
  unsigned char correct_auth =
      0; // variable for storing attempted authentications
  unsigned char auth_started =
      0; // flag for telling us that someone is trying to authenticate
  ...
  while (1) {
    ...
    else if ((((correct_auth & 0x8) >> 3) + ((correct_auth & 0x4) >> 2) +
              ((correct_auth & 0x2) >> 1) + (correct_auth & 0x1)) >=
             lock_threshold) {
      lcd_moveto(20);
      lcd_stringout("unlocked.");
      lock_timeout = 0;
